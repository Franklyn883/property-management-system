...
class ManagerAssignmentSerializer(serializers.ModelSerializer, ValidationMixin):
    ...
    def validate_assigned_properties(self, value):
        ...
        if not isinstance(value, list):
            raise serializers.ValidationError("Assigned properties must be a list")
        
        for prop in value:
            if not isinstance(prop, dict):
                raise serializers.ValidationError("Each assigned property must be an object")
            
            required_fields = ['property_id', 'property_address', 'assignment_date']
            for field in required_fields:
                if field not in prop:
                    raise serializers.ValidationError(f"Assigned property missing required field: {field}")
    ...
    def validate_management_contracts(self, value):
        ...
        if not isinstance(value, list):
            raise serializers.ValidationError("Management contracts must be a list")
        
        for contract in value:
            if not isinstance(contract, dict):
                raise serializers.ValidationError("Each management contract must be an object")
            
            required_fields = ['contract_id', 'property_id', 'start_date', 'commission_rate']
            for field in required_fields:
                if field not in contract:
                    raise serializers.ValidationError(f"Management contract missing required field: {field}")
...
class ManagerAssignmentEntrySerializer(serializers.Serializer, ValidationMixin):
    ...
    management_fee = serializers.DecimalField(max_digits=5, decimal_places=2, required=False)
    ...
    def validate_management_fee(self, value):
        ...
        if value is not None and (value < 0 or value > 100):
            raise serializers.ValidationError("Management fee must be between 0 and 100")
        return value


class ManagerMaintenanceEntrySerializer(serializers.Serializer, ValidationMixin):
    """
