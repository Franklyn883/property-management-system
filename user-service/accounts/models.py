...
class FailedLoginAttempt(models.Model):
    ...
    def is_ip_locked(cls, ip_address):
        ...
        recent_attempts = cls.objects.filter(
            ip_address=ip_address, timestamp__gte=cutoff_time
        ).count()
        return (
            recent_attempts >= 10
        )  # Lock after 10 failed attempts in 15 minutes
    ...
    def is_email_locked(cls, email):
        ...
        recent_attempts = cls.objects.filter(
            email=email, timestamp__gte=cutoff_time
        ).count()
        return (
            recent_attempts >= 5
        )  # Lock after 5 failed attempts in 30 minutes
...
class BlockedIP(models.Model):
    ...
    def is_ip_blocked(cls, ip_address):
        ...
        now = timezone.now()
        return (
            cls.objects.filter(ip_address=ip_address)
            .filter(
    ...
    def cleanup_expired_blocks(cls):
        ...
        now = timezone.now()
        cls.objects.filter(is_permanent=False, blocked_until__lt=now).delete()
