...
class AgentLicenseSerializer(serializers.ModelSerializer, ValidationMixin):
    ...
    def validate_license_id(self, value):
        ...
        if not value:
            raise serializers.ValidationError(
                "License ID is required for agents"
            )

        # Basic validation for license ID format
        if len(value) < 5:
            raise serializers.ValidationError(
                "License ID must be at least 5 characters long"
            )
    ...
    def validate_license_documents(self, value):
        ...
        if not isinstance(value, list):
            raise serializers.ValidationError(
                "License documents must be a list"
            )

        for doc in value:
            if not isinstance(doc, dict):
                raise serializers.ValidationError(
                    "Each license document must be an object"
                )
            ...
            for field in required_fields:
                if field not in doc:
                    raise serializers.ValidationError(
                        f"License document missing required field: {field}"
                    )
...
class AgentAgencySerializer(serializers.ModelSerializer, ValidationMixin):
    ...
    def validate_clients_managed_count(self, value):
        ...
        if value is not None and value < 0:
            raise serializers.ValidationError(
                "Clients managed count cannot be negative"
            )
        return value
