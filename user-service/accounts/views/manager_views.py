...
class ManagerAssignmentViewSet(ViewSet):
    ...
    def update_assignments(self, request):
        ...
        serializer = ManagerAssignmentSerializer(
    ...
    def get_assignment_stats(self, request):
        ...
        active_assignments = sum(
            ...
            if assignment.get("status") != "terminated"
        )

        # Calculate total management fees
        total_fees = sum(
            assignment.get("management_fee", 0) for assignment in assignments
        )
        ...
        stats = {
            ...
            "average_fee": (
                round(total_fees / total_assignments, 2)
...
class ManagerMaintenanceViewSet(ViewSet):
    ...
    def get_maintenance_stats(self, request):
        ...
        pending_requests = sum(
            1 for req in requests if req.get("status") == "pending"
        )
        in_progress_requests = sum(
            1 for req in requests if req.get("status") == "in_progress"
        )
        completed_requests = sum(
            1 for req in requests if req.get("status") == "completed"
        )

        # Calculate total estimated costs
        total_estimated_cost = sum(
            req.get("estimated_cost", 0) for req in requests
        )
        ...
        priority_counts = {
            ...
            "medium": sum(
                1 for req in requests if req.get("priority") == "medium"
            ),
            "high": sum(1 for req in requests if req.get("priority") == "high"),
            "urgent": sum(
                1 for req in requests if req.get("priority") == "urgent"
            ),
        }
...
class ManagerDashboardView(APIView):
    ...
    def get(self, request):
        ...
        property_stats = {
            ...
            "properties_by_status": self._get_property_status_breakdown(
                assignments
            ),
        }
        ...
        contract_stats = {
            ...
            "total_commission": sum(
                contract.get("commission_rate", 0) for contract in contracts
            ),
